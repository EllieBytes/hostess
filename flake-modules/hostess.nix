...
in
{
  options.hostess = {
    ...
    # Injected inputs.
    # See the README sec 3 (Injected Inputs). for a more comprehensive description.
    nixpkgs = mkOption {
      type = types.raw;
    ...
    in
    {
      flake =
        let
          ...
          subdirs = dir: attrNames (filterAttrs (_: t: t == "directory") (builtins.readDir dir));

          safeImport = path: default: if pathExists path then import path else default;
          ...
          resolveModule =
            modulesDir: name:
            let
              p = (fileOrDirectory modulesDir name);
            in
            if pathExists p then p else throw "hostess: Couldn't find module ${name} at ${p}";

          resolveProfile =
            profilesDir: name:
            let
              p = (fileOrDirectory profilesDir name);
            in
            if pathExists p then p else throw "hostess: Couldn't find profile ${name} at ${p}";

          hostessLib = cfg.nixpkgs.lib.extend (
            import ../lib/default.nix {
              inherit cfg;
            }
          );

          hostessLibHome = cfg.nixpkgs.lib.extend (
            import ../lib/home.nix {
              inherit cfg;
            }
          );
          ...
          buildHost =
            ...
            let
              ...
              commonModule = d: optional (pathExists (d + "/common.nix")) (d + "/common.nix");
                  ...
                  home-manager.users = builtins.listToAttrs (
                    map (
                      ...
                      let
                        ...
                        homeCommon = optional (pathExists (cfg.homeModulesDir + "/common.nix")) (
              ...
              modules =
                ...
                ++ (
                  if diskoOk then
                    ...
                    builtins.warn
                      "Host ${hostname} has a disko config, but disko was never given to Hostess. not configuring disko"
                      [ ]
                  else
                    [ ]
                )
